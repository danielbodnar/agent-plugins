/**
 * containers / templates-containers-template
 *
 * Source page: Containers
 * Docs subpath: docs/containers/get-started.mdx
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest --template=cloudflare/templates/containers-template
 *   [yarn] yarn create cloudflare --template=cloudflare/templates/containers-template
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli containers templates-containers-template           # print
 *   ./scripts/cli containers templates-containers-template --run     # execute
 *   ./scripts/cli containers templates-containers-template --json    # metadata
 *   ./scripts/cli containers templates-containers-template --target-dir=my-project
 *
...
export const meta = {
  product: "containers",
  subcommand: "templates-containers-template",
  description: "In this guide, we will build and push a container image alongside your Worker code. By default, this process uses[Docker ↗](https://www.docker.com/) to do so.",
  page_title: "Containers",
  template_id: "cloudflare/templates/containers-template",
  target_dir: null,
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- --template=cloudflare/templates/containers-template",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest --template=cloudflare/templates/containers-template"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare --template=cloudflare/templates/containers-template"
    }
  ],
} as const;
