 ...
 *
 * Source page: First Worker
 * Docs subpath: docs/learning-paths/workers/get-started/first-worker.mdx
 * Default target dir: first-worker
 *
 *
 * Primary command:
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest first-worker
 *   [yarn] yarn create cloudflare first-worker
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli learning-paths first-worker           # print
 *   ./scripts/cli learning-paths first-worker --run     # execute
 *   ./scripts/cli learning-paths first-worker --json    # metadata
 *   ./scripts/cli learning-paths first-worker --target-dir=my-project
 *
...
export const meta = {
  ...
  subcommand: "first-worker",
  description: "",
  page_title: "First Worker",
  template_id: null,
  target_dir: "first-worker",
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- first-worker",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest first-worker"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare first-worker"
    }
  ],
} as const;
