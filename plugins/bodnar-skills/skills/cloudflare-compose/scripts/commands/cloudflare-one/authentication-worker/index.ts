/**
 * cloudflare-one / authentication-worker
 *
 * Source page: CORS
 * Docs subpath: docs/cloudflare-one/access-controls/applications/http-apps/authorization-cookie/cors.mdx
 * Default target dir: authentication-worker
 *
 *
 * Primary command:
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest authentication-worker
 *   [yarn] yarn create cloudflare authentication-worker
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli cloudflare-one authentication-worker           # print
 *   ./scripts/cli cloudflare-one authentication-worker --run     # execute
 *   ./scripts/cli cloudflare-one authentication-worker --json    # metadata
 *   ./scripts/cli cloudflare-one authentication-worker --target-dir=my-project
 *
...
export const meta = {
  ...
  subcommand: "authentication-worker",
  description: "",
  page_title: "CORS",
  template_id: null,
  target_dir: "authentication-worker",
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- authentication-worker",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest authentication-worker"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare authentication-worker"
    }
  ],
} as const;
