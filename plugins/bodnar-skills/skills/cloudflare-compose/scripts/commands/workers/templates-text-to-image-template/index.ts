/**
 * workers / templates-text-to-image-template
 *
 * Source page: Templates
 * Docs subpath: docs/workers/get-started/quickstarts.mdx
 * Template: cloudflare/templates/text-to-image-template
 *
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest --template=cloudflare/templates/text-to-image-template
 *   [yarn] yarn create cloudflare --template=cloudflare/templates/text-to-image-template
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli workers templates-text-to-image-template           # print
 *   ./scripts/cli workers templates-text-to-image-template --run     # execute
 *   ./scripts/cli workers templates-text-to-image-template --json    # metadata
 *   ./scripts/cli workers templates-text-to-image-template --target-dir=my-project
 *
...
export const meta = {
  product: "workers",
  subcommand: "templates-text-to-image-template",
  description: "[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/cloudflare/templates/tree/main/text-to-image-template)",
  page_title: "Templates",
  template_id: "cloudflare/templates/text-to-image-template",
  target_dir: null,
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- --template=cloudflare/templates/text-to-image-template",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest --template=cloudflare/templates/text-to-image-template"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare --template=cloudflare/templates/text-to-image-template"
    }
  ],
} as const;
