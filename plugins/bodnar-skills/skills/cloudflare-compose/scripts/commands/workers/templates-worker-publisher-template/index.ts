/**
 * workers / templates-worker-publisher-template
 *
 * Source page: Templates
 * Docs subpath: docs/workers/get-started/quickstarts.mdx
 * Template: cloudflare/templates/worker-publisher-template
 *
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest --template=cloudflare/templates/worker-publisher-template
 *   [yarn] yarn create cloudflare --template=cloudflare/templates/worker-publisher-template
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli workers templates-worker-publisher-template           # print
 *   ./scripts/cli workers templates-worker-publisher-template --run     # execute
 *   ./scripts/cli workers templates-worker-publisher-template --json    # metadata
 *   ./scripts/cli workers templates-worker-publisher-template --target-dir=my-project
 *
...
export const meta = {
  product: "workers",
  subcommand: "templates-worker-publisher-template",
  description: "[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/cloudflare/templates/tree/main/worker-publisher-template)",
  page_title: "Templates",
  template_id: "cloudflare/templates/worker-publisher-template",
  target_dir: null,
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- --template=cloudflare/templates/worker-publisher-template",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest --template=cloudflare/templates/worker-publisher-template"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare --template=cloudflare/templates/worker-publisher-template"
    }
  ],
} as const;
