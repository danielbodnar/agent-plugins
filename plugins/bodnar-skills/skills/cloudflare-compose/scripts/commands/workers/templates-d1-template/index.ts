 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest --template=cloudflare/templates/d1-template
 *   [yarn] yarn create cloudflare --template=cloudflare/templates/d1-template
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli workers templates-d1-template           # print
 *   ./scripts/cli workers templates-d1-template --run     # execute
 *   ./scripts/cli workers templates-d1-template --json    # metadata
 *   ./scripts/cli workers templates-d1-template --target-dir=my-project
 *
...
export const meta = {
  product: "workers",
  subcommand: "templates-d1-template",
  description: "[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/cloudflare/templates/tree/main/d1-template)",
  page_title: "Templates",
  template_id: "cloudflare/templates/d1-template",
  target_dir: null,
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- --template=cloudflare/templates/d1-template",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest --template=cloudflare/templates/d1-template"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare --template=cloudflare/templates/d1-template"
    }
  ],
} as const;
