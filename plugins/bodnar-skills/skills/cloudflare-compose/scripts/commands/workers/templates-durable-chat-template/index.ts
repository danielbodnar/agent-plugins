/**
 * workers / templates-durable-chat-template
 *
 * Source page: Templates
 * Docs subpath: docs/workers/get-started/quickstarts.mdx
 * Template: cloudflare/templates/durable-chat-template
 *
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest --template=cloudflare/templates/durable-chat-template
 *   [yarn] yarn create cloudflare --template=cloudflare/templates/durable-chat-template
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli workers templates-durable-chat-template           # print
 *   ./scripts/cli workers templates-durable-chat-template --run     # execute
 *   ./scripts/cli workers templates-durable-chat-template --json    # metadata
 *   ./scripts/cli workers templates-durable-chat-template --target-dir=my-project
 *
...
export const meta = {
  product: "workers",
  subcommand: "templates-durable-chat-template",
  description: "[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/cloudflare/templates/tree/main/durable-chat-template)",
  page_title: "Templates",
  template_id: "cloudflare/templates/durable-chat-template",
  target_dir: null,
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- --template=cloudflare/templates/durable-chat-template",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest --template=cloudflare/templates/durable-chat-template"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare --template=cloudflare/templates/durable-chat-template"
    }
  ],
} as const;
