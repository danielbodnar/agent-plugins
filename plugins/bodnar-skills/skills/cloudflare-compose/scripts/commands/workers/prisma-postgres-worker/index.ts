/**
 * workers / prisma-postgres-worker
 *
 * Source page: Securely access and upload assets with Cloudflare R2
 * Docs subpath: docs/workers/tutorials/using-prisma-postgres-with-workers.mdx
 * Default target dir: prisma-postgres-worker
 *
 ...
 *   ./scripts/cli workers prisma-postgres-worker           # print
 *   ./scripts/cli workers prisma-postgres-worker --run     # execute
 *   ./scripts/cli workers prisma-postgres-worker --json    # metadata
 *   ./scripts/cli workers prisma-postgres-worker --target-dir=my-project
 *
...
export const meta = {
  product: "workers",
  subcommand: "prisma-postgres-worker",
  description: "Begin by using [C3](https://developers.cloudflare.com/pages/get-started/c3/) to create a Worker project in the command line:",
  page_title: "Securely access and upload assets with Cloudflare R2",
  template_id: null,
  target_dir: "prisma-postgres-worker",
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest prisma-postgres-worker -- --type=hello-world --ts=true --git=true --deploy=false",
  alternates: [],
} as const;
