 ...
 * Docs subpath: docs/workers/tutorials/build-a-slackbot.mdx
 * Default target dir: slack-bot
 *
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest slack-bot
 *   [yarn] yarn create cloudflare slack-bot
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli workers slack-bot           # print
 *   ./scripts/cli workers slack-bot --run     # execute
 *   ./scripts/cli workers slack-bot --json    # metadata
 *   ./scripts/cli workers slack-bot --target-dir=my-project
 *
...
export const meta = {
  product: "workers",
  subcommand: "slack-bot",
  description: "There are many different event types that can be enabled for your webhook. Selecting **Issues** will send every issue-related event to your webhook, including when issues are opened, edited, deleted, and more. If you would like to expand your Slack bot application in the future, you can select more of these events after the tutorial.",
  page_title: "Build a QR code generator",
  template_id: null,
  target_dir: "slack-bot",
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- slack-bot",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest slack-bot"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare slack-bot"
    }
  ],
} as const;
