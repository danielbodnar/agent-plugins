 ...
 * r2 / consumer-worker
 *
 * Source page: Log and store upload events in R2 with event notifications
 * Docs subpath: docs/r2/tutorials/upload-logs-event-notifications.mdx
 * Default target dir: consumer-worker
 *
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest consumer-worker
 *   [yarn] yarn create cloudflare consumer-worker
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli r2 consumer-worker           # print
 *   ./scripts/cli r2 consumer-worker --run     # execute
 *   ./scripts/cli r2 consumer-worker --json    # metadata
 *   ./scripts/cli r2 consumer-worker --target-dir=my-project
 *
...
export const meta = {
  product: "r2",
  subcommand: "consumer-worker",
  description: "Before you enable event notifications for `example-upload-bucket`, you need to create a [consumer Worker](https://developers.cloudflare.com/queues/reference/how-queues-works/#create-a-consumer-worker) to receive the notifications.",
  page_title: "Log and store upload events in R2 with event notifications",
  template_id: null,
  target_dir: "consumer-worker",
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- consumer-worker",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest consumer-worker"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare consumer-worker"
    }
  ],
} as const;
