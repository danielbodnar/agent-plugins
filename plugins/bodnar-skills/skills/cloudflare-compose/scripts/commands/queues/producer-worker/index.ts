/**
 * queues / producer-worker
 *
 * Source page: Observability
 * Docs subpath: docs/queues/get-started.mdx
 * Default target dir: producer-worker
 *
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest producer-worker
 *   [yarn] yarn create cloudflare producer-worker
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli queues producer-worker           # print
 *   ./scripts/cli queues producer-worker --run     # execute
 *   ./scripts/cli queues producer-worker --json    # metadata
 *   ./scripts/cli queues producer-worker --target-dir=my-project
 *
...
export const meta = {
  product: "queues",
  subcommand: "producer-worker",
  description: "You will access your queue from a Worker, the producer Worker. You must create at least one producer Worker to publish messages onto your queue. If you are using [R2 Bucket Event Notifications](https://developers.cloudflare.com/r2/buckets/event-notifications/), then you do not need a producer Worker.",
  page_title: "Observability",
  template_id: null,
  target_dir: "producer-worker",
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- producer-worker",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest producer-worker"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare producer-worker"
    }
  ],
} as const;
