/**
 * queues / resend-rate-limit-queue
 *
 * Source page: Handle rate limits of external APIs
 * Docs subpath: docs/queues/tutorials/handle-rate-limits/index.mdx
 * Default target dir: resend-rate-limit-queue
 *
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest resend-rate-limit-queue
 *   [yarn] yarn create cloudflare resend-rate-limit-queue
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli queues resend-rate-limit-queue           # print
 *   ./scripts/cli queues resend-rate-limit-queue --run     # execute
 *   ./scripts/cli queues resend-rate-limit-queue --json    # metadata
 *   ./scripts/cli queues resend-rate-limit-queue --target-dir=my-project
 *
...
export const meta = {
  product: "queues",
  subcommand: "resend-rate-limit-queue",
  description: "Before you can use Queues, you must enable it via [the Cloudflare dashboard ↗](https://dash.cloudflare.com/?to=/:account/workers/queues). You need a Workers Paid plan to enable Queues.",
  page_title: "Handle rate limits of external APIs",
  template_id: null,
  target_dir: "resend-rate-limit-queue",
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- resend-rate-limit-queue",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest resend-rate-limit-queue"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare resend-rate-limit-queue"
    }
  ],
} as const;
