/**
 * agents / my-slack-agent
 *
 * Source page: Securing MCP servers
 * Docs subpath: docs/agents/guides/slack-agent.mdx
 * Default target dir: my-slack-agent
 *
 *
 * Primary command:
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest my-slack-agent
 *   [yarn] yarn create cloudflare my-slack-agent
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli agents my-slack-agent           # print
 *   ./scripts/cli agents my-slack-agent --run     # execute
 *   ./scripts/cli agents my-slack-agent --json    # metadata
 *   ./scripts/cli agents my-slack-agent --target-dir=my-project
 *
...
export const meta = {
  product: "agents",
  subcommand: "my-slack-agent",
  description: "",
  page_title: "Securing MCP servers",
  template_id: null,
  target_dir: "my-slack-agent",
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- my-slack-agent",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest my-slack-agent"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare my-slack-agent"
    }
  ],
} as const;
