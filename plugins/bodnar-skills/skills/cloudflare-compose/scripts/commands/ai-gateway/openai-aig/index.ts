/**
 * ai-gateway / openai-aig
 *
 * Source page: Deploy a Worker that connects to OpenAI via AI Gateway
 * Docs subpath: docs/ai-gateway/tutorials/deploy-aig-worker.mdx
 * Default target dir: openai-aig
 *
 ...
 *
 * Alternate runtimes:
 *   [pnpm] pnpm create cloudflare@latest openai-aig
 *   [yarn] yarn create cloudflare openai-aig
 *
 * Invoke via the dispatcher:
 *   ./scripts/cli ai-gateway openai-aig           # print
 *   ./scripts/cli ai-gateway openai-aig --run     # execute
 *   ./scripts/cli ai-gateway openai-aig --json    # metadata
 *   ./scripts/cli ai-gateway openai-aig --target-dir=my-project
 *
...
export const meta = {
  product: "ai-gateway",
  subcommand: "openai-aig",
  description: "For this tutorial, we will be using the OpenAI provider-specific endpoint, so select OpenAI in the dropdown and copy the new endpoint.",
  page_title: "Deploy a Worker that connects to OpenAI via AI Gateway",
  template_id: null,
  target_dir: "openai-aig",
  primary_runtime: "npm",
  primary_command: "npm create cloudflare@latest -- openai-aig",
  alternates: [
    {
      "runtime": "pnpm",
      "command": "pnpm create cloudflare@latest openai-aig"
    },
    {
      "runtime": "yarn",
      "command": "yarn create cloudflare openai-aig"
    }
  ],
} as const;
